...
public class ParentExamController {
	...
	@GetMapping("parent/{studentNumber}/exam/no")
	...
	public String childexamList(@AuthenticationPrincipal CustomUserDetails principal,
	                             @PathVariable Integer studentNumber,
	                             @PathVariable Integer lectureNumber,
	                             Model model) {
	    int parentNumber = principal.getMember().getIdNumber();
	    List<RegistrationVO> childlecture = parentExamService.childlecture(lectureNumber);
	    System.out.println(childlecture);
	    model.addAttribute("childlecture", childlecture);
	...
	@ResponseBody
	public int getExamListCount(@PathVariable Integer lectureNumber, @RequestParam(required = false) String testTitle,
	...
	@ResponseBody
	public void autoGradeExam(@RequestBody List<GradingResult> gradingResultList,
			@PathVariable Integer participateNumber) {
	...
	public String viewChildExamResult(@PathVariable Integer studentNumber, @PathVariable Integer lectureNumber,
			@PathVariable Integer testNumber, @RequestParam Integer participateNumber, Model model) {
